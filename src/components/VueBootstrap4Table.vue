<template>
  ...
  <div :class="{card:card_mode}">
    ...
    max-height: 500px; border: 1px solid #dee2e6">
          <table class="table" :class="tableClasses" style="margin-bottom: 0">
            <thead>
                ...
                <template v-for="(column) in columns" :slot="'vbt-'+getCellSlotName(column)">
                  ...
                  >
                    <a
                      ...
                      :href="'http://local.therapyappointment.com:8888/index.cfm/people:billing/account/user_id/'+ row.id"
                      title="linked ho gaya bhai"
                    >{{getValueFromRow(row,column.name)}}</a>
                    <span v-else>{{getValueFromRow(row,column.name)}}</span>
                  </slot>
                </template>
              </row>
                    ...
                    <div class="col-md-4">
                      <div class="text-right justify-content-center">
                        ...
                        <template v-if="selected_rows_info && isSelectable">
                          ...
                          >{{selectedItemsCount}} rows selected</slot>
                        </template>
                      </div>
                    </div>
            ...
            <tfoot>
              <tr>
                <th
                  v-for="item in vbt_dfoot"
                  style="position: -webkit-sticky;
    position: sticky;
    bottom: 0;
    z-index: 2; background:#fff; border: 1px solid #dee2e6;"
                >{{item}}</th>
              </tr>
            </tfoot>
          </table>
...
export default {
  ...
  mounted() {
    ...
    this.vbt_dfoot = cloneDeep(this.footer);
    let self = this;
  ...
  methods: {
    ...
    initFilterQueries() {
      this.vbt_columns.forEach(vbt_column => {
        ...
        if (vbt_column.filter.type == "simple") {
          ...
          let selected_options = vbt_column.filter.options
    ...
    simpleFilter(value, filter_text, config) {
      ...
      let is_case_sensitive = has(config, "case_sensitive")
        ...
        if (!this.server_mode) {
          newVal.forEach(customFilter => {
            if (customFilter.name) {
              let index = this.query.filters.findIndex(
